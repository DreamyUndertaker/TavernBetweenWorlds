...
namespace LaunhcerForMinecraft
{
    /*public class Download
    {
        ...
        public async Task ExecuteAsync(DownloadOptions options)
        {
            ...
            var resource = await DiskClient.MetaInfo.GetInfoAsync(new YandexDisk.Client.Protocol.ResourceRequest() { Path = options.Source }).ConfigureAwait(false);
            if (resource == null)
            {
            ...
            using (var downloadStream = await DiskClient.Files.DownloadAsync(link).ConfigureAwait(false))
            {
                ...
                {
                    case YandexDisk.Client.Protocol.ResourceType.Dir:
                        if (options.Unzip)
                        {
                        ...
                        break;
                    case YandexDisk.Client.Protocol.ResourceType.File:
